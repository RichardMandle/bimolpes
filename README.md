...
# You'll need:
Numpy, Mayavi, scipy, Gaussian G16 (or G09) or ORCA, access to a HPC<br>

# What it does, and how it does it:
Bimolpes works in either `write`, `read`, `inspect`, or `plot` mode. Argument parsing is done by the BimolPESParser class in the command_parser.py module. Some default values are stored (and can be edited) in `config.ini`<br><br>

With `python bimolpes.py write`:<br>
This takes an initial geometry (molecule #1; Gaussian .log file specified as `-mol`) which you have previously geometry optimised at some apropriate level, and generates a user specified set of translation coordinates (`-x` `-y` or `-z`: either supply two values, min:max, or one, -val:val) The resolution is controlled with the `-res` flag; you can supply 1 (x=y=z), 2 (x, y=z) or 3 (x, y, z) values here. The analysis code is written in such a way that you can combine an initial low resolution grid with higher resolution scans over volumes of interest, so a first pass with `-res` =1 or =2 is fine .<br><br> 
...
Each set of coordinates is then written to a Gaussian .gjf file, or an ORCA .inp file (if the `-orca` flag is passed); these include fragment information for each molecule, permitting counterpoise correction (enabled for Gaussian only currently). You likely do need empirical dispersion dispersion (e.g. GD3BJ). You should use the same functional/basis set as the initial optimisation. The program writes these to the directory given by `-out` (or defaults to -inp1_inp2) and will write to .zip file (contains .gjf and .sh files) unless told not to by passing the `-zip` flag. If previous grid files are found in the same output directory then these are backed up to a .zip file, unless the `-backoff` flag is called. A full list of option flags which are used in writing Gaussian files is given below:<br>
`-orca` = Switches to "ORCA mode" and will produce .inp input files and handle reading ORCA .out files. The flags below are handled apropriately, e.g. `-cpu` will set %nprochsared in Gaussian mode (default), and %pal nprocs in Orca mode;<br>
`-cpu` = number of CPU cores (int, defaults to 4);<br>
`-mem` = amount of RAM to use (in GB; int, defaults to 4);<br>
`-route` = Gaussian route section to use (str, defaults to #T B3LYP cc-pVTZ EmpiricalDispersion=GD3BJ counterpoise=2);<br>
...
<br><br>
The code is set up to auto generate .sh files for HPC submission via SGE-qsub, and will utilize task arrays for multiple jobs as apropriate. There are some options for cleaining up unwanted *.sh.* files, core* files from failed Gaussian Jobs, and for converting *.chk files to *.fchk files _via_ formchk if needed (For Gaussian only). A future update will handle SLURM jobs.
<br><br>

With `python bimolpes.py inspect`:
This mode is designed to read a set of constructed .gjf (or .inp) files and produce a 3D map of the displacement vectors. This enables the user to quickly check that full coverage has been achieved. If there are "holes" in the produced inspection surface and you need to fill them, then re-enter `python bimolpes.py write` mode and use larger grid spacings, or reduce `-min_dist` (or increase `-max_dist`!). This produces a .npz file much like `read` mode, note that hte energy data is just the index of each grid spacing so as to allow differentiation of points by colouration. An example is given below:<br> 

`python bimolpes.py inspect -filename rm734_anti`
Produces the file inspection.npz file ready for visualisation. <br>
`python bimolpes.py plot -filename inspection.npz`
Get the plotting engine to visualise the ipsection surface; note, you can use many of the commands listed for `plot`, below. This produces an 3D image:

![image](https://github.com/user-attachments/assets/e293f3df-d255-4389-9e8a-c8868c0dca68)
...
`-plt_glyph` Lets you choose different glpyhs for plotting through mayavi: e.g. `cube`, `cone`, `sphere`... see mlab docs!<br>
...
`-plt_minpoint` specify the midpoint of the colourmap; for negative values use quotation marks (e.g. "-0.05"). A midpoint of zero allows attractive/repulsive interactions to be nicely visualised when using a _diverging colourmap_ e.g. `-plt_cmap bwr -plt_midpoint 0 -plt_min "-15" -plt_max 4`<br>
