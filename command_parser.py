...
class BimolPESParser:
    ...
    def setup_read_parser(self):
        ...
        read_parser.add_argument('-nosave', action='store_const', const=False, default=False, help='Don\'t save output to .npz')
    ...
    def setup_plot_parser(self):
        ...
        plot_parser.add_argument('-plt_show_nan', action='store_true', default=self.config.getboolean('PLOT','PltShowNan', fallback = False), help='Show NaN values in the resulting volumetric plots')

        plot_parser.add_argument('-plt_style', type=str, default=self.config.get('PLOT', 'PltStyle', fallback='points'), help='Select between \'surface\', \'points\', \'wireframe\', etc; see mlab (str; fallback = points)')
        plot_parser.add_argument('-plt_glyph', type=str, default=self.config.get('PLOT', 'PltGlyph', fallback='cube'), help='Select between \'cube\', \'cone\', \'sphere\', etc; see mlab (str; fallback = cube)')
        ...
        plot_parser.add_argument('-vis_plt', action='store_const', const=True, default=False, help='if true, trigger doing a vis_plt # TO DO - what on earth is a vis plt?')
        ...
        add_gaussian_options(plot_parser, self.config)
...
def add_gaussian_options(parser, config):
...
def convert_to_none_or_float(value):
    # tiny function (does it need to be a function) to return a float if any val is passed, else return None.
    if value == 'None':
        return None
    try:
        return float(value)
